...
class Unit:
    def init(self, canvas, x, y, speed, padding, bot, default_image):
...
class Tank(Unit):
        def init(self, canvas, row, col, bot=True):
            super().init(canvas, col * world.BLOCK_SIZE, row * world.BLOCK_SIZE, 2, 8, bot, "tank_up")
            if bot:
...
class Missile(Unit):
            def init(self, canvas, owner):
                super().init(canvas, owner.get_x(), owner.get_y(), 6, 20, False, 'missile_up')
